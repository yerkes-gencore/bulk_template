...
# analysis$samplefileIDs <- dir(file.path(
#   analysis$config$rootDir, analysis$config$alignmentDir))
# analysis$samplefileIDs <- analysis$samplefileIDs[
#   analysis$samplefileIDs %in% analysis$sampleTable$FileID]

analysis$samplefileIDs <- analysis$sampleTable$FileID

analysis$sampleSTARReads <- sapply(analysis$samplefileIDs, function(sid) {
  file <- dir(
    file.path(analysis$config$rootDir, analysis$config$alignmentDir),
    pattern = sid, full.names = TRUE)
  read_tsv(paste0(file,"/",sid,analysis$config$STARreadSuffix),
...
analysis$mapBins <- do.call(
  cbind,
  sapply(analysis$sampleTable$FileID, 
    function(sid){
      analysis$sampleSTARReads[[sid]][
        ,paste0(analysis$config$libraryType,"_count")][c(1:4),]
    }))
rownames(analysis$mapBins) <- analysis$sampleSTARReads[[1]][c(1:4),][["gene_id"]]
colnames(analysis$mapBins) <-
  recode(
    as.character(
      unlist(
        strsplit(
          analysis$sampleTable$SampleID,
...
normLogCounts <- log(
  DESeq2::counts(analysis$ddsDrop, normalized = TRUE)[
    rowMins(DESeq2::counts(analysis$ddsDrop))>0,]
  )
...
dir.create("outputs")
## sample table
write_csv(analysis$sampleTable, path = paste0("outputs/sample_table_",
                                              analysis$config$analysis,".csv"),
          col_names = TRUE)
## raw counts w/ gene symbols
write_csv(as.data.frame(raw_counts) %>% 
            rownames_to_column(var = "gene_id"),
          path = paste0("outputs/raw_count_",
...
write_csv(as.data.frame(ensembleRawCount) %>% 
            rownames_to_column(var = "EnsemblID"),
          path = paste0("outputs/raw_count_EnsemblID_", 
...
write_tsv(data.frame(Name = str_remove(rownames(analysis$assayRlogForGSEA), "[.].*"),
                     Description = "na", analysis$assayRlogForGSEA),
          path = paste0("outputs/rlog_forGSEA_", analysis$config$analysis, ".txt"))
...
analysis$clsLinesGroup <-
  c(paste0(c(length(analysis$rldDrop$Group),
           ...
           collapse = " "),
    paste0(c("#",unique(as.vector(analysis$rldDrop$Group))), collapse = " "),
...
```{r}
saveRDS(analysis, 'saved_objects/analysis_post_QC.Rds')
```
