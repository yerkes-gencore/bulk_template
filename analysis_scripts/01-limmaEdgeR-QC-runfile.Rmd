...
```{r}
if (!dir.exists(here("saved_rds_objects/report-01/"))) {
  dir.create(here("saved_rds_objects/report-01/"))
}
write_rds(bulk, here("saved_rds_objects/report-01", "bulk-p23224_Matthew-no_design_yet.rds"))
```
...
```{r, fig.width=8, fig.height=8}
ggplotMDS(bulk$dge, group = "grp.day", gene.selection = "pairwise",
          sampleID = "sampleID", color = "grp", path = "SubjectID")
```
...
## Write `bulk` rds
```{r}
if (!dir.exists(here("saved_rds_objects/", subdir))) {
  dir.create(here("saved_rds_objects/", subdir))
}

bulk_prefilt %>%
  write_rds(., here("saved_rds_objects/", subdir, paste0("bulk_prefilt-", getDate(), ".rds")))
bulk %>%
  write_rds(., here("saved_rds_objects/", subdir, paste0("bulk-", getDate(), ".rds")))
```
