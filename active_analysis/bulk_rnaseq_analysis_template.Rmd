...
```{r}
analysis <- readRDS('saved_objects/analysis_post_QC.Rds')
...
gsea_plot_data <- gsea_plot_data %>%
  separate(ID, c("ID"), extra = 'drop', sep=' ') %>%
  mutate(ID = substr(ID, 6, 100))

ggplot(gsea_plot_data) + 
  geom_point(aes(x=ID, y=pathway, size=-log(pval), color=NES)) +
  scale_color_gradient2(low="blue", mid="white",high="red", midpoint=0,
                        breaks=c(-2,-1,0,1,2), limits=c(min(gsea_plot_data$NES,-1),
                                                        max(gsea_plot_data$NES,1))) +
  theme_classic(11) +
  ...
  labs(x="Comparison", y="Gene set", color = "Normalized\nenrichment\nscore",
       size="Nom p-val", title="Top Pathways") +
  scale_radius(name="NOM p-val", range=c(1,8),
...
analysis_shiny <- analysis[c('mapBins', 'sampleTable', 'config', 'ddsDrop', 'RLE_norm', 'RLE_raw', 'pca', 'rldDrop')]
comparisons_table <- read.csv('config/comparisons.txt', sep='\t')
analysis_shiny$comparisons_table <- comparisons_table
analysis_shiny$config$designBaselineCategory <- 'Time'
analysis_shiny$config$designBaselineValue <- 'D0'
saveRDS(analysis_shiny, file='saved_objects/p22167_Tiffany_Analysis.RDS')
```
