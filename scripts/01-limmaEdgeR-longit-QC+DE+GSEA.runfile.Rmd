...
```{r}
bulk$md$project <- "p23224_Matthew"
...
manifest_tbl <- read_csv("/yerkes-cifs/runs/analyst/micah/Analysis/2023_Analyses/p23224_Matthew/p23224_Matthew_Analysis/notes/SampleManifest_micah.csv", show_col_types = FALSE)
...
```{r}
keep.exprs <- filterByExpr(bulk$dge, design = stats::model.matrix(~ 0 + grp.day, data = bulk$md$sampleTable))
```

### 2.1 - Gene filtering by expression
```{r, fig.height = 6, fig.width = 16}
sampleID_colors <- tibble(
  sampleID = unique(bulk$dge$samples$sampleID)) %>%
  mutate(color = "black")
...
pcas <- lapply(dims_list, function(dims) {
  ggplotMDS(bulk$dge, dims = dims, group = "grp.day", gene.selection = "common", show.labels = FALSE,
          sampleID = "sampleID", color = "grp", shape = "day", path = "subjectID", alpha = 0.50)
})
...
mdss <- lapply(dims_list, function(dims) {
  ggplotMDS(bulk$dge, dims = dims, group = "grp.day", gene.selection = "pairwise", show.labels = FALSE,
          sampleID = "sampleID", color = "grp", shape = "day", path = "subjectID", alpha = 0.50)
})
...
*Note* that technically we could model animal as a fixed effect (See "7.5 Time series experiment with repeated mouse measurements nested within treatments" at https://bioconductor.org/packages/release/workflows/vignettes/RNAseq123/inst/doc/designmatrices.html#versus-2-group-comparisons) but I think that's only possible for studies with two factors (i.e. time and some other factor) if there are only two time points / repeated measures and one is treated as a baseline. Otherwise how would you specify contrasts for particular days? This was my failed attempt to do this for this study.
```{r}
# design_alt <- stats::model.matrix(~ 0 + SubjectID, data = bulk$md$sampleTable)
# colnames(design_alt) <- gsub("SubjectID", "", colnames(design_alt))
# design_alt <- cbind(design_alt, P14 = bulk$md$sampleTable$day=="P14" & bulk$md$sampleTable$grp=="grp3")
# design_alt <- cbind(design_alt, D00 = bulk$md$sampleTable$day=="D00" & bulk$md$sampleTable$grp=="grp3")
# design_alt <- cbind(design_alt, D03 = bulk$md$sampleTable$day=="D03" & bulk$md$sampleTable$grp=="grp3")
# design_alt <- cbind(design_alt, D07 = bulk$md$sampleTable$day=="D07" & bulk$md$sampleTable$grp=="grp3")
# design_alt <- cbind(design_alt, D28 = bulk$md$sampleTable$day=="D28" & bulk$md$sampleTable$grp=="grp3")
# 
# contr.matrix.alt <- makeContrasts(
#   # Change-from-baseline group effects
#   D00vsP14_grp3vsgrp2 = D00 - (grp2.D00 - grp2.P14),
#   D03vsP14_grp3vsgrp2 = (grp3.D03 - grp3.P14) - (grp2.D03 - grp2.P14),
#   D07vsP14_grp3vsgrp2 = (grp3.D07 - grp3.P14) - (grp2.D07 - grp2.P14),
#   D28vsP14_grp3vsgrp2 = (grp3.D28 - grp3.P14) - (grp2.D28 - grp2.P14),
#   levels = design_alt
#   )
```
...
# # If we want the contrasts in the same spreadsheet side-by-side
...
# If we want to see the contrasts in separate spreadsheets
resultsTable_for_export <- resultsTable %>%
  split(., f = resultsTable$contrast)

c(list("Summary" = n_deg_summary), 
  resultsTable_for_export) %>%
writexl::write_xlsx(here("outputs/", subdir, resultsTable_filename))
```
...
sample_tbl <- bulk$dge$samples %>%
  dplyr::select(sampleID, grp.day) %>%
  mutate(color = "black") %>%
  dplyr::rename(group = grp.day)
gene_tbl <- resultsTable %>%
  getTopDegTbl(., contrast_i = colnames(contr.matrix)[1], groupID = "group", 
...
### Render
```{r}
rmarkdown::render(input = here("scripts/01-limmaEdgeR-longit-QC+DE+GSEA.format.Rmd"),
