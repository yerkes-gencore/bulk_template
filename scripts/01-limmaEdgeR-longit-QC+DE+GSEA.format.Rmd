...
bulk$md$sampleTable %>%
  select(fileID, sampleID, subjectID, grp.day) %>%
  group_by(grp.day) %>%
...
We treated grp.day and batch as fixed effects. The model formula can be represented as follows:

`expression ~ grp.day + subjectID`

Where `grp.day` is parameterized as a fixed effect and `subjectID` as a random effect (since this is a repeated measures experiment), modeled using `edgeR::voomLmFit()` with block set to `subjectID`.
...
gene_tbl <- resultsTable %>%
  getTopDegTbl(., contrast_i = "{{contrast_i}}", groupID = "grp.day", 
