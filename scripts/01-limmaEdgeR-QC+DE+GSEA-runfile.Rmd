...
Note that the example dataset here is a cross-sectional study with several contrast of interest, but this script . Just uncomment the code chunk section 4.2.
...
n_deg_summary <- resultsTable %>% 
  group_by(contrast) %>%
    ...
    summarize(signif_down = sum(qvalue < fdr & l2fc < 0),
              nonsignif = sum(qvalue >= fdr),
              signif_up = sum(qvalue < fdr & l2fc > 0)) %>%
...
### Render
```{r}
rmarkdown::render(input = here("scripts/cross-01-QC+DE+GSEA-format.Rmd"),
