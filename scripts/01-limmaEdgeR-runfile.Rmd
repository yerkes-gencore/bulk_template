...
ggplotMDS1 <- function(dge, group = "group", sampleID = "sampleID", gene.selection = "common", ...) {
...
```{r}
# Plot voom by group plot
# vbg_data <- getVoomByGroupData(bulk, group = "grp")

vbg_plot <- getVoomByGroupData(bulk, group = "grp") %>%
    dplyr::filter(.[["cluster"]] == clust) %>%
    plotVoomByGroupData1()
```
...
```{r}
ggplotMDS1(bulk$dge, group = "grp", gene.selection = "common")
```
...
```{r}
ggplotMDS1(bulk$dge, group = "grp", gene.selection = "pairwise")
```
...
contr.levels_tbl <- contr.levels %>%
  ...
  mutate(level = str_remove(level, "^grp"))

contr.levels_vec <- contr.levels_tbl$level %>% unique() #%>% str_remove("^grp_chl")

nSamplesPerLevel <- contr.levels_tbl %>%
  mutate(level = str_remove(level, "^grp")) %>%
  full_join(count(bulk$dge$samples, grp, name = "nSamples"), by = c("level" = "grp"))

limitingLevels_tbl <- nSamplesPerLevel %>%
  select(-level) %>%
  pivot_wider(names_from = type, values_from = nSamples)

limitingLevels_tbl %>% knitr::kable()
```
