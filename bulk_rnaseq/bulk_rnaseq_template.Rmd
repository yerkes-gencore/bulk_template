...
# a <- function(data, var){
#   return(data %>% group_by(.data[[var]]))#
# }
# for (i in 1:length(analysis$config$sampleGrouping)){
#   a <- analysis$config$sampleGrouping[[i]]
#   exp.design <- exp.design %>% arrange(.data[[a]])  
# }
# tmp <- analysis$config$sampleGrouping
# exp.design %>% arrange(.data[[tmp]])
# exp.design <- do.call(a, as.list(analysis$config$sampleGrouping))
exp.design <- exp.design %>% arrange(across(all_of(analysis$config$sampleGrouping)))
...
#analysis$resultsDrop <- results(analysis$ddsDrop, name = "Group_LiproxposFAC_vs_Cont", alpha = 0.05)3
...
#for (i in 1:(length(colnames(analysis$sampleTable))-2)){
  ...
  analysis$pca <- pcaPlotGKT(analysis$vst,
                                  intgroup = names(colData(analysis$dds)),
                                  xpc = 1, ypc = 2) +
...
pairwise_comparisons <- function(pair){
  ...
  gVolPlot <- gVolData %>%
  arrange(!is.na(.data[[colnames(gVolData)[5]]]),desc(.data[[colnames(gVolData)[5]]])) %>%
...
# tmp2 <- results_objs[1]
# tmp_func <- function(df1, df2, pair1, pair2){
#   merge(df1, df2,)
# }
#tmp3 <- vector("list", (length(pair_order)-1))

#tmp4 <- lapply(results_objs, paste(colnames()))
if (length(pair_order) > 1){
  ...
  # for (i in 2:(length(pair_order)-1)){
  #   tmp2 <- merge(results_objs[i-1], results_objs[i], by=0, all=T, suffixes=c(pair_order[i-1], pair_order[i]))
  #   tmp3[[i-1]] <- tmp2 
  #}
...
baseline_means <- rowMeans2(tmp[, analysis$rldDrop@colData$Group %in% analysis$config$designBaseline])
tmp1 <- tmp - baseline_means
Heatmap(tmp1, show_row_names = F, heatmap_legend_param = list(title="RLE"))#[,!(analysis$rldDrop@colData$Group %in% analysis$config$designBaseline)])
